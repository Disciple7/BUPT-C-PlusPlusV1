...
int state_testmake(vector<string>& wordList)
{
	fstream wordFile;
	if (DEV_MODE)
		wordFile.open("D:\\coding\\C++Final\\C++Final_1\\Debug\\wordList.csv", ios::in | ios::out | ios::app);
	else
...
void flush_tester(int nowUserPos, int wordAdd, vector<tester>& testerList)
{
	...
	fstream testerFile;
	if (DEV_MODE)
		testerFile.open("D:\\coding\\C++Final\\C++Final_1\\Debug\\testerList.csv", ios::in | ios::out);
	else
